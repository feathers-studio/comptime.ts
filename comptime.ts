...
export async function getComptimeReplacements(tsconfigPath?: string): Promise<Replacements> {
	const config = tsconfigPath ?? ts.findConfigFile(".", ts.sys.fileExists, "tsconfig.json");
...
export async function applyComptimeReplacements(tsconfigPath: string, outdir: string, replacements: Replacements) {
	const config = tsconfigPath ?? ts.findConfigFile(".", ts.sys.fileExists, "tsconfig.json");
	if (!config) throw new Error("Could not find tsconfig.json");
